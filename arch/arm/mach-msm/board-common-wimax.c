...
{
	printk(KERN_INFO "[WIMAX] %s counter:%d\n", __func__, counter);
...
/*
	0:Disbale this feature
	1:Disable irq when system suspend
	2:Disable IRQ when WiMAX CLK off
*/
int mmc_wimax_set_disable_irq_config(int on)
{
	printk(KERN_INFO "[WIMAX] %s on:%d\n", __func__, on);
	mmc_wimax_disable_irq_config = on;
	return 0;
}
EXPORT_SYMBOL(mmc_wimax_set_disable_irq_config);

int mmc_wimax_get_disable_irq_config(void)
{
	return mmc_wimax_disable_irq_config;
}
EXPORT_SYMBOL(mmc_wimax_get_disable_irq_config);
