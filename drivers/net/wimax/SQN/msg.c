...
#define SQN_PRT_MODULE_NAME	"[WIMAX] wimax_prt"
#define SQN_MSG_PREFIX		"[WIMAX] [MSG]"
...
int sqn_pr_info_dump(char *prefix, unsigned char *data, unsigned int len)
{
	...
	if (i < len_) {
		if (i % width == 0) {
			if (len_ >= 40 && !bHandle) { /* ARP [ */
				if ((((unsigned char *)(data))[i+12] == 0x08) &&
					...
					if (opCode == 1) { /* request */
						printk(KERN_INFO "%s: Who has ", SQN_MSG_PREFIX);
					...
					/* Destination MAC */
					printk(KERN_INFO "%s: Dst MAC: ", SQN_MSG_PREFIX);
					for (pos = 0; pos < 6; pos++) {
						if (pos < 5)
								printk("%02x:", ((unsigned char *)(data))[pos]);
						else
					...
					/* Source MAC */
					printk(KERN_INFO "%s: Src MAC: ", SQN_MSG_PREFIX);
			...
			if (len_ >= 34 && !bHandle) { /* ICMP [ */
				...
				if ((((unsigned char *)(data))[i+12] == 0x08) &&
					...
					/* Source IP */
					printk(KERN_INFO "%s: Src IP: ", SQN_MSG_PREFIX);
					...
					/* Destination IP */
					printk(KERN_INFO "%s: Dst IP: ", SQN_MSG_PREFIX);
			...
			if (len_ >= 12 && !bHandle) { /* IPv6 [ */
				...
				if (   (((unsigned char *)(data))[i+12] == 0x86) &&
					...
					/* Source IP */
					printk(KERN_INFO "%s: Dst MAC: ", SQN_PRT_MODULE_NAME);
					for (pos = 0; pos < 6; pos++) {
						if (pos < 5)
							printk("%d:", ((unsigned char *)(data))[pos]);
						else
					...
					/* Destination IP */
					printk(KERN_INFO "%s: Src MAC: ", SQN_PRT_MODULE_NAME);
					for (pos = 6; pos < 12; pos++) {
						if (pos < 11)
							printk("%d:", ((unsigned char *)(data))[pos]);
						else
			...
			if (len_ >= 34 && !bHandle) { /* Unknown UDP [ */
				...
				if ((((unsigned char *)(data))[i+12] == 0x08) &&
					...
					printk(KERN_INFO "%s: Port: %d%d\n", SQN_MSG_PREFIX, ((unsigned char *)(data))[i+34], ((unsigned char *)(data))[i+35]);
				}
			} /* Unknown ] */
		...
		/* printk(KERN_INFO "%s: %02x ", ((unsigned char *)(data))[i++]); */
		/* if ((i % width == 0) || (i == len_))
			printk(KERN_INFO "\n"); */
	}
...
void sqn_pr_thp_info_dump(char *prefix, unsigned char *data, unsigned int len)
{
	...
	if (i < len_) {
		if (len_ >= 29) { /* len_ [ */
			if ((((unsigned char *)(data))[i+12] == 0x00) &&  /* THP [ */
				 (((unsigned char *)(data))[i+13] == 0x18)) {

				if ((((unsigned char *)(data))[i+26] == 0x00) &&
					 (((unsigned char *)(data))[i+27] == 0x00) &&
					 (((unsigned char *)(data))[i+28] == 0x00)) { /* THP service [ */
						if ( (((unsigned char *)(data))[i+29] == 0x00) )
							printk(KERN_INFO "%s: [THP] ServiceId:%d - Init\n", SQN_PRT_MODULE_NAME, ((unsigned char *)(data))[i+29] );
						else if ( (((unsigned char *)(data))[i+29] == 0x01) )
							printk(KERN_INFO "%s: [THP] ServiceId:%d - GetThpServices\n", SQN_PRT_MODULE_NAME, ((unsigned char *)(data))[i+29] );
						else if ( (((unsigned char *)(data))[i+29] == 0x02) )
							printk(KERN_INFO "%s: [THP] ServiceId:%d - GetInit\n", SQN_PRT_MODULE_NAME, ((unsigned char *)(data))[i+29] );
						else if ( (((unsigned char *)(data))[i+29] == 0x03) )
							printk(KERN_INFO "%s: [THP] ServiceId:%d - theException\n", SQN_PRT_MODULE_NAME, ((unsigned char *)(data))[i+29] );
						else if ( (((unsigned char *)(data))[i+29] == 0x04) )
							printk(KERN_INFO "%s: [THP] ServiceId:%d - FilterThpNtf\n", SQN_PRT_MODULE_NAME, ((unsigned char *)(data))[i+29] );
						else if ( (((unsigned char *)(data))[i+29] == 0x05) )
							printk(KERN_INFO "%s: [THP] ServiceId:%d - FatalError\n", SQN_PRT_MODULE_NAME, ((unsigned char *)(data))[i+29] );
						else
							printk(KERN_INFO "%s: [THP] ServiceId:%d\n", SQN_PRT_MODULE_NAME, ((unsigned char *)(data))[i+29] );

					} /* THP service ] */
				else
					printk(KERN_INFO "%s: [THP] ServiceId:%x %x %x %x\n", SQN_PRT_MODULE_NAME, ((unsigned char *)(data))[i+26], ((unsigned char *)(data))[i+27], ((unsigned char *)(data))[i+28], ((unsigned char *)(data))[i+29]);
			} /* THP ] */
		} /* len_ ] */

		if (len_ > MAX_THP_DUMP_LEN)
			len_ = MAX_THP_DUMP_LEN;
...
void sqn_pr_info_dump_rawdata_lite(char *prefix, unsigned char *data, unsigned int len)
{
	...
	if (len_ > MAX_LITE_DUMP_LEN)
		len_ = MAX_LITE_DUMP_LEN;
...
int sqn_filter_packet_check(char *prefix, unsigned char *data, unsigned int len)
{
	...
	if (i < len_) {
		...
		if (len_ >= 34 && !bHandle) {	/* Daytime (RFC867) [ */
				/* IP: 0x0800, TCP: 0x06, port: 0x000d (13) */
				if ((((unsigned char *)(data))[i+12] == 0x08) &&
					(((unsigned char *)(data))[i+13] == 0x00) &&
					(((unsigned char *)(data))[i+23] == 0x06) &&
					((((unsigned char *)(data))[i+36] == 0x00) && (((unsigned char *)(data))[i+37] == 0x0d))) {
					bHandle = 1;
					bFilter = 1;
				}
		}
...
void printTime32(u_char *data)
{
	...
	printk(KERN_INFO "%s: %d (", SQN_MSG_PREFIX, t);
	if (t > SPERW) {
		printk(KERN_INFO "%s: %dw", SQN_MSG_PREFIX, t / (SPERW));
		t %= SPERW;
	}
	if (t > SPERD) {
		printk(KERN_INFO "%s: %dd", SQN_MSG_PREFIX, t / (SPERD));
		t %= SPERD;
	}
	if (t > SPERH) {
		printk(KERN_INFO "%s: %dh", SQN_MSG_PREFIX, t / (SPERH));
		t %= SPERH;
	}
	if (t > SPERM) {
		printk(KERN_INFO "%s: %dm", SQN_MSG_PREFIX, t / (SPERM));
		t %= SPERM;
	}
	if (t > 0)
		printk(KERN_INFO "%s: %ds", SQN_MSG_PREFIX, t);
	printk(KERN_INFO "%s: )", SQN_MSG_PREFIX);
}
