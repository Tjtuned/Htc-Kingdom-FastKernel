...
printk(KERN_DEBUG " at %lld (%d-%02d-%02d %02d:%02d:%02d.%09lu UTC)\n", \
...
msmsdcc_disable_clocks(struct msmsdcc_host *host, int deferr)
{
	...
	if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
		if (!host->clks_on) {
			printk(KERN_INFO "[WIMAX] [MMC] in %s - %s: clks_on is OFF\n", __func__, mmc_hostname(host->mmc));
			return;
		}
	}

	if (host->curr.mrq) {
		printk(KERN_INFO "[MMC] %s: curr.mrq != NULL", mmc_hostname(host->mmc));
		return;
	}
...
#endif
				pr_info("[WIMAX] [MMC] %s: Disable clocks in %s\n", mmc_hostname(host->mmc), __func__);
			}
#endif
...
#if SDC_CLK_VERBOSE
	if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {	
		pr_info("[WIMAX] [MMC] %s: Bus clock timer expired\n", mmc_hostname(host->mmc));
	}
#endif
...
msmsdcc_enable_clocks(struct msmsdcc_host *host)
{
	...
	if (!host->clks_on) {
		...
		if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
			...
			host->clks_on = 1;
#endif
			pr_info("[WIMAX] [MMC] %s: Enable clocks\n", mmc_hostname(host->mmc));
		}
#endif
		...
		if (rc) {
			...
			if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
				host->clks_on = 0;
				pr_info("[WIMAX] [MMC] %s: Enable pclk clocks Failed! \n", mmc_hostname(host->mmc));
			}
#endif
			return rc;
		}
		...
		if (rc) {
			...
			if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
				host->clks_on = 0;
				pr_info("[WIMAX] [MMC] %s: Enable clk clocks Failed! \n", mmc_hostname(host->mmc));
			}
#endif
			return rc;
		}
	...
	if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
		if ((host->sdcc_during_suspend)) {
			if (!bDumpLog)
				pr_info("[WIMAX] [MMC] %s: sdcc_during_suspend == 1, WiMAX do not disable host wakeup!\n", mmc_hostname(host->mmc));
			bDumpLog = 1;
		} else {
			/* pr_info("%s: sdcc_during_suspend == 0, WiMAX disable host wakeup\n", mmc_hostname(host->mmc)); */
...
#ifdef CONFIG_WIMAX
	if (is_wimax_platform(host->plat)) {
		if (IS_ERR(mrq->data) || IS_ERR(host->curr.data)) {
			printk(KERN_INFO "[WIMAX] [MMC] %s mrq->ptr is invalid!\n", __func__);
			return;
		}
	}
#endif
...
#ifdef CONFIG_WIMAX
	if (is_wimax_platform(host->plat)) {
		if (IS_ERR(host->mmc) || IS_ERR(mrq)) {
			printk(KERN_INFO "[WIMAX] [MMC] %s mrq is invalid!\n", __func__);
			return;
		}
	}
#endif
...
msmsdcc_pio_read(struct msmsdcc_host *host, char *buffer, unsigned int remain)
{
	...
	if ( (remain < 4) ) {
		...
		while (msmsdcc_readl(host, MMCISTATUS) & MCI_RXDATAAVLBL) {
			...
			if (is_wimax_platform(host->plat)) {
				if (IS_ERR(ptr)) {
					printk(KERN_ERR "[WIMAX] [MMC] in %s: %s ptr is invalid!\n", __func__, mmc_hostname(host->mmc));
					count = 0;
					break;
				}
			}
...
msmsdcc_irq(int irq, void *dev_id)
{
	...
	do {
		if (!host->clks_on) {
			...
			if (!is_wimax_platform(host->plat)) {
				/* For wimax, we have wakeup source to turn on CLK to avoid race condition in here. */
...
msmsdcc_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
		if (host->curr.mrq != NULL) {
			printk(KERN_WARNING "[WIMAX] [MMC] %s: %s curr.mrq != NULL,", __func__, mmc_hostname(host->mmc));
		}
	} 
	else
...
msmsdcc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->clock) {
		if (!host->clks_on && is_svlte_platform(host->plat)) {
			...
			clk_enable(host->clk);
			host->clks_on = 1;
			printk(KERN_DEBUG "%s: %s clks_on %d, irqmask 0x%x\n",
...
msmsdcc_init_dma(struct msmsdcc_host *host)
{
	...
	if (is_mmc_platform(host->plat))
		host->dma.channel = DMOV_NAND_CHAN;
	else
...
msmsdcc_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	if (mmc) {
		...
		if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
			host->sdcc_during_suspend = 1;
			bDumpLog = 0;
			pr_info("[WIMAX] [MMC] %s: in %s, host->sdcc_during_suspend = 1 \n", mmc_hostname(host->mmc), __func__);
		}
#endif
		if (host->clks_on && !is_svlte_platform(host->plat)) {
#if SDC_CLK_VERBOSE
			...
			if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
				msmsdcc_disable_clocks(host, 0);
				pr_info("[WIMAX] [MMC] %s: msmsdcc_disable_clocks(%s, 0) in %s\n", mmc_hostname(host->mmc), mmc_hostname(host->mmc), __func__);
			} else {
				/* For suspend case */
				clk_disable(host->clk);
				clk_disable(host->pclk);
...
#ifdef CONFIG_WIMAX
			if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
				pr_info("[WIMAX] [MMC] %s: Enable clocks in %s\n", mmc_hostname(host->mmc), __func__);
				host->sdcc_during_suspend = 0;
				bDumpLog = 0;
				pr_info("[WIMAX] [MMC] %s: in %s, host->sdcc_during_suspend = 0 \n", mmc_hostname(host->mmc), __func__);
				pr_info("[WIMAX] [MMC] %s: msmsdcc_enable_clocks(%s) in %s\n", mmc_hostname(host->mmc), mmc_hostname(host->mmc), __func__);
			}
#endif
			msmsdcc_enable_clocks(host);
#ifdef CONFIG_WIMAX
